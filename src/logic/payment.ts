...
export class Payment {
    ...
    constructor (config: ConfigPayment) {
        ...
        this._tokenA = {
            info: config.tokenA,
            contract: new ethers.Contract(config.tokenA.address[idNetw], token_abi, this._provider),
            erc20: new Token(
                this._networkId,
        ...
        this._tokenB = {
            info: config.tokenB,
            contract: new ethers.Contract(config.tokenB.address[idNetw], token_abi, this._provider),
            erc20: new Token(
                this._networkId,
        ...
        this._contractPermit = new ethers.Contract(this._addressPermit, permit2, this._provider)
    ...
    public async checkAllowance (token: 'A' | 'B', type: 'permit' | 'polus'): Promise<BigNumber> {
        let contr = this._tokenA.contract
        if (token === 'B') {
            contr = this._tokenB.contract
        }

        const toPermit = type === 'permit' ? this._addressPermit : this._addressPolusContract

        try {
            const allow: BigNumber = await contr.allowance(this._addressUser, toPermit)
    ...
    (token: 'A' | 'B', type: 'router' | 'polus'): Promise<Permit2AllowanceType | undefined> {
        let contr = this._tokenA.contract
        if (token === 'B') {
            contr = this._tokenB.contract
        }

        const toRouter = type === 'router' ? this._addressRouter : this._addressPolusContract

        try {
            const allow: Permit2AllowanceType = await this._contractPermit.allowance(
                this._addressUser, contr.address, toRouter
            )
    ...
    public static async getAllAmountIn (amountOut: string, outToken: ListToken): Promise<ListTokens> {
        const localFull = fullListTokens

        const newLocalFull = await getPriceToken(localFull)

        const amountOutUSDPrice = newLocalFull.filter(token => token.namePrice === outToken.namePrice)[0].price
    ...
    (address: string | 'permit', type: 0 | 1 = 1): Promise<ethers.providers.TransactionRequest | any> {
        if (type === 0) {
            const data = this._tokenA.contract.interface.encodeFunctionData(
                'approve',
                [ address === 'permit' ? this._addressPermit : address, ethers.constants.MaxUint256 ]
            )
        ...
        return {
            ...
            abi: token_abi,
            functionName: 'approve',
            args: [ address === 'permit' ? this._addressPermit : address, ethers.constants.MaxUint256 ]
        }
    }
