...
export async function getPriceToken (tokensList: ListTokens): Promise<ListTokens> {
    ...
    const data = await axios.get(
        `https://api.coingecko.com/api/v3/simple/price?ids=${listNamed.join(',')}&vs_currencies=usd`
    )
...
export function weiToEthNum (nanoAmount: BigNumber | string | BigInt, decimals: number): number {
    return Number(ethers.utils.formatUnits(BigNumber.from(nanoAmount), decimals))
}

export function weiToEth (nanoAmount: BigNumber | string, decimals: number): string {
